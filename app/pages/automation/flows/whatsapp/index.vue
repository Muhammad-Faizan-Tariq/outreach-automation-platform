...
const flows = ref([
  { id: '1', name: 'Dubai Marina Follow-up', status: 'active', steps: 5, enrolled: 142, replied: 38, created_at: '2026-06-01' },
  { id: '2', name: 'JBR Property Inquiry', status: 'draft', steps: 3, enrolled: 0, replied: 0, created_at: '2026-06-10' },
])
...
<template>
  <div class="px-6 py-8 max-w-6xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-8">
      <div>
        ...
        <p class="text-sm text-muted mt-0.5">
          Automated follow-up sequences sent via WhatsApp
        </p>
      </div>
    ...
    <div v-if="flows.length === 0" class="text-center py-24">
      <UIcon name="i-lucide-git-branch" class="w-12 h-12 text-muted mx-auto mb-3" />
      <p class="text-lg font-semibold text-highlighted mb-1">
        No flows yet
      </p>
      <p class="text-sm text-muted mb-4">
        Create your first automated WhatsApp follow-up sequence
      </p>
      <UButton icon="i-lucide-plus" @click="openModal">
        Create Flow
      </UButton>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      <NuxtLink
        v-for="flow in flows"
        :key="flow.id"
        :to="`/automation/flows/whatsapp/${flow.id}`"
        class="block"
      >
            ...
            <div class="flex items-start justify-between gap-2">
              <div class="flex items-center gap-2 min-w-0">
                <div class="w-8 h-8 rounded-lg bg-green-500/10 flex items-center justify-center shrink-0">
                  <UIcon name="i-lucide-git-branch" class="w-4 h-4 text-green-600" />
                </div>
    ...
    <!-- Create flow modal -->
    <UModal v-model:open="showModal" title="New WhatsApp Flow" :ui="{ width: 'sm:max-w-lg' }">
      <template #body>
        <div class="space-y-5 p-1">
          ...
          <div>
            ...
            <UInput
              v-model="form.name"
              placeholder="e.g. Dubai Marina Follow-up"
              autofocus
            />
          </div>
